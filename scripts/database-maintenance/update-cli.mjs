...
export async function runCli(
  ...
  try {
    ...
    if (result.status === "cancelled") {
      output.write("updateをキャンセルしました。\n");
      return 0;
    }
    output.write(
      `updateを実行しました: ${result.label} (${result.databaseName})\n`,
    );
    ...
    if (error instanceof UpdateExecutionError) {
      errorOutput.write(
        `${error.resultUnknown ? "update実行結果は不明です" : "updateに失敗しました"}: ${error.message}\n`,
      );
      if (error.backupPath) {
        errorOutput.write(`変更前backup: ${error.backupPath}\n`);
      }
    } else {
      errorOutput.write(`updateに失敗しました: ${error.message}\n`);
    }
    return 1;
  }
}
...
function usage() {
  return `Database maintenance task JSON update

Usage:
  scripts/local-update.sh <tasks.json> | --help
  scripts/remote-update.sh <tasks.json> | --help
